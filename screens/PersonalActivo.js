...
export default class PersonalActivo extends React.Component{
	...
	CargarPersonal(){
		fetch(routes.baseurl.url+'/prueba/contactos.php',
		  {
			  method: 'post',
			  headers: {
			    'Accept': 'application/json, text/plain, */*',
			  ...
			  body: JSON.stringify({action:'cargarguardas'})
      	...
      	.then(respuesta => {
      		...
      		if(respuesta == "empty"){
      			...
      			this.setState({contactos:respuesta});
      		}
      	});
	}
	...
	render(){
		const { style, isfocus, text, contactos }  = this.state;
		return(
			<View>
								...
								return(
									<View style={{flexDirection:'row',width:WIDTH,height:70,backgroundColor:'white',borderColor:'#f2f2f2',borderBottomWidth:2}} key={index+1}>
										...
										<View style={{position:'relative',flexDirection:'column'}}>
											...
											<Text style={{position:'relative',left:15,color:'#c3c3c3',width:(WIDTH-110)}}>{data.email}</Text>
											<Text style={{position:'relative',left:15,color:'#c3c3c3',width:(WIDTH-110)}}>#{data.telefono}</Text>
											<View style={{position:'absolute',right:-35,bottom:17,padding:10,backgroundColor:(data.status == 1 ? '#f0f0f0' : 'green'),borderRadius:6}}><Text style={{fontSize:12,color:'white'}}>{data.status == 1 ? "INACTIVO" : 'ACTIVO'}</Text></View>
										</View>
									</View>
								);
							}
